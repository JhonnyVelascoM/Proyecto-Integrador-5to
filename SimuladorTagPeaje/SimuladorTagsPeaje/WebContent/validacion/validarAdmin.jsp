...
<% String correo = request.getParameter("txtnomAdmin");
	...
	ResultSet rs = new Conexion().Consulta("select * from administrador where correo_administrador='"+correo+"'");
	try{
	rs.next();
	if(rs.first()!=false){
		if(!rs.getString("claveusuario").equals(pass)){ 
			...
			out.print("<a id=\"boton\" href=\"../presentacion/ingresoAdmin.jsp\">Regresar</a>");
			out.print("</div>");
		}else{ 
			...
			response.sendRedirect("../presentacion/sesionAdmin.jsp");
		}
	}else{
		out.print("<label>Administrador no Registrado</label></div>");
		out.print("<div style=\"text-align: center;margin-top:30px;\">");
		out.print("<a id=\"boton\" href=\"../presentacion/ingresoAdmin.jsp\">Regresar</a>");
		out.print("</div>");
	}
